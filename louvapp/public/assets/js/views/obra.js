    ...
    $.ajax({
        ...
        for (const [puesto, total] of Object.entries(puestosTotales)) {
            ...
            nuevoElemento.innerHTML = `<div class="avatar-md rounded">
                <span class="avatar-title bg-success-lighten text-success border border-success rounded-circle h3 my-0 total-${total}">${total}</span>
                ${puesto}
            </div>`;
...
function graficaPrincipal(registro){
    ...
    series: []
      ...
      registro.registro.forEach((resultado) => {
        ...
        if (!datosAcomodados.categories.includes(empresa)) {
          ...
          // Inicializar la serie en 0 para esta empresa
        ...
        if (serieExistente) {
          // Acumular trabajadores en la serie existente
          serieExistente.data[datosAcomodados.categories.indexOf(empresa)] += trabajadores;
        } else {
          // Crear una nueva serie
          const nuevaSerie = {
            name: puesto,
            data: Array(datosAcomodados.categories.length).fill(0),
          };
      
          // Colocar los trabajadores en la posición correspondiente
    ...
    const options = {
        ...
        title: {
            text: 'FUERZA DE TRABAJO DEL DÍA'
        },
        plotOptions: {
            bar: {
                horizontal: true,
        ...
        series: [], // Las series se cargarán dinámicamente según los datos
        xaxis: {
